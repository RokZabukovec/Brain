<template>
  <div class="category">
    ...
    <div class="container">
      <div class="row">
        <div @click="openModal(command)" class="col-sm-12 col-md-6 p-0 command-container" v-for="command in category.commands" v-bind:key="command.id">
          <Command :command="command"></Command>
        </div>
      </div>
    </div>
    
    <b-modal v-bind:id="category.name + '-' + category.id.toString()" title="New command" hide-footer>
...
class Category extends Vue {
  ...
  onCommandSave(categoryId) {
    ...
    axios.post(url, this.commandForm)
        .then(response => {
          console.log('COMMANDS UNDEFINED', response);
          console.log('COMMANDS UNDEFINED', this.$props.category);
